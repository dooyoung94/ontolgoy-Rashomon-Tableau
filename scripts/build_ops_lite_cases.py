...
def _trace_dependency_edges(normal_traces: pd.DataFrame) -> list[CausalEdge]:
    ...
    if not needed.issubset(normal_traces.columns):
        return []
    parent = normal_traces[["trace_id", "span_id", "service_name"]].dropna().rename(
        columns={"span_id": "parent_span_id", "service_name": "parent_service"}
    )
    child = normal_traces[["trace_id", "parent_span_id", "service_name"]].dropna().rename(
...
def _service_stats(traces: pd.DataFrame, logs: pd.DataFrame) -> dict[str, dict]:
    ...
    if "service_name" in traces.columns:
        for service, group in traces.dropna(subset=["service_name"]).groupby("service_name"):
            service = str(service)
            durations = pd.to_numeric(group.get("duration", pd.Series(dtype=float)), errors="coerce").dropna()
    ...
    if "service_name" in logs.columns:
        for service, group in logs.dropna(subset=["service_name"]).groupby("service_name"):
            service = str(service)
            level = group.get("level", pd.Series("", index=group.index)).fillna("").astype(str).str.upper()
            out.setdefault(service, {})["log_error_rate"] = float(level.isin(["ERROR", "FATAL", "CRITICAL"]).mean()) if len(group) else 0.0
    return out


def _metric_shift(normal: pd.DataFrame, abnormal: pd.DataFrame, service: str) -> tuple[float, str, float | None]:
    ...
    for metric in common:
        ...
        if low.endswith(".time") or low.endswith("_time"):
            continue
        nv = pd.to_numeric(nsvc.loc[nsvc["metric"].astype(str) == metric, "value"], errors="coerce").dropna()
        av = pd.to_numeric(asvc.loc[asvc["metric"].astype(str) == metric, "value"], errors="coerce").dropna()
        ...
        if score > best[0]:
            ...
            if "_time_s" in asvc.columns:
                vals = pd.to_numeric(asvc.loc[asvc["metric"].astype(str) == metric, "value"], errors="coerce")
...
def _evidence_for_case(
    ...
    services = sorted(set(ns) | set(ac) | set(normal_metrics.get("service_name", pd.Series(dtype=str)).dropna().astype(str)) | set(abnormal_metrics.get("service_name", pd.Series(dtype=str)).dropna().astype(str)))
    ...
    for service in services:
        ...
        if np95 > 0 and ap95 > np95:
            ...
            if lat_score >= 0.10:
                onset = abnormal_start
                if "service_name" in abnormal_traces.columns and "_time_s" in abnormal_traces.columns and "duration" in abnormal_traces.columns:
                    grp = abnormal_traces[abnormal_traces["service_name"].astype(str) == service]
                    d = pd.to_numeric(grp["duration"], errors="coerce")
                    hit = grp.loc[d > max(float(n.get("p99", np95)) * 1.25, np95 * 1.5), "_time_s"]
                    if len(hit):
                        onset = float(hit.min())
                eid += 1
                evidence.append(Evidence(
        ...
        if log_score >= 0.10:
            onset = abnormal_start
            if "service_name" in abnormal_logs.columns and "_time_s" in abnormal_logs.columns and "level" in abnormal_logs.columns:
...
def _symptoms(known_edges: list[CausalEdge], evidence: list[Evidence]) -> list[str]:
    ...
    for e in evidence:
        anomaly[e.node] = max(anomaly.get(e.node, 0.0), e.abnormality)
    sources = {e.source for e in known_edges}
    targets = {e.target for e in known_edges}
...
def _gold_from_causal_graph(graph: dict) -> tuple[list[str], list[CausalEdge], list[str]]:
    ...
    for edge in graph.get("edges", []):
        s = service({"component": edge.get("source")})
        t = service({"component": edge.get("target")})
        if s and t and s != t:
            pairs.add((s, t))
    edges = [CausalEdge(s, "causal_propagates_to", t) for s, t in sorted(pairs)]
